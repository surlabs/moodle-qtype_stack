import regexallmatch from '../../corsscripts/ascii/extractors/regexallmatch.js';

describe('regexallmatch extractor', () => {
    describe('guard clauses', () => {
        test('returns ERROR when operation is undefined', () => {
            expect(regexallmatch('any raw', null, undefined)).toBe('ERROR');
        });

        test('returns ERROR when operation is null', () => {
            expect(regexallmatch('any raw', null, null)).toBe('ERROR');
        });

        test('returns ERROR when operation.regex is missing', () => {
            expect(regexallmatch('any raw', null, {})).toBe('ERROR');
        });

        test('returns ERROR when operation.regex is empty', () => {
            expect(regexallmatch('any raw', null, { regex: '' })).toBe('ERROR');
        });
    });
    ...
    describe('matching lines', () => {
        test('returns all matching lines as JSON', () => {
            ...
            const result = regexallmatch(raw, null, operation);
